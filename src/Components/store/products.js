...
const initailState = {
    products: [
        { name: 'xBox', category: 'Electronics', image: xbox, description: 'The Xbox is a series of video game consoles known for their gaming performance, multimedia capabilities, and online gaming services.', price: 350, stock: 5, inCard: false },
        { name: 'PlayStation', category: 'Electronics', image: PlayStation, description: 'PlayStation is a popular brand of video game consoles and related products developed and produced by Sony Interactive Entertainment, a division of Sony Corporation', price: 346, stock: 2, inCard: false },
        { name: 'PC', category: 'Electronics', image: pc, description: '"PC" is an abbreviation that stands for "Personal Computer." It refers to a broad category of computing devices designed for individual use, as opposed to larger, more specialized computers used by organizations or data centers.', price: 600, stock: 3, inCard: false },
    ...
    switch (type) {
        ...
        case 'ADD_CART':
            const updatedProducts = state.products.map((product) => {
                if (product.name === payload.name && product.stock > 0) {
                    return { ...product, stock: product.stock - 1, inCard: true };
                }
                return product;
            });
        ...
        case 'DELETE_CART':
            const unDelete = state.products.map((product) => {
                if (product.name === payload.name) {
                    return { ...product, stock: product.stock + 1, inCard: false };
                }
                return product;
            });
